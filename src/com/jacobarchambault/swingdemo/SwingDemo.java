...
class SwingDemo extends JFrame {
	...
	CardPanel cards = new CardPanel(cardLayout);
	CenterPanel centerPanel = new CenterPanel();
	JRadioButton[] rBs = new JRadioButton[] { new JRadioButton("Show Labels/Fields", true),
			new JRadioButton("Hide Labels/Fields", false) };

	SwingDemo(final String title) {
		super(title);
	...
	private void addRadioButtonsToCenterPanel() {
		...
		rBs[0].addActionListener(e -> cardLayout.first(cards));
		rBs[1].addActionListener(e -> cardLayout.last(cards));
	}
}
